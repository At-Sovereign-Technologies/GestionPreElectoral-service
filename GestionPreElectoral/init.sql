...
CREATE TABLE IF NOT EXISTS gestion_pre_electoral.registros_censo (
	...
	CONSTRAINT fk_registro_censo_ciudadano FOREIGN KEY (ciudadano_id)
		REFERENCES gestion_pre_electoral.ciudadanos (id),
	CONSTRAINT ck_registro_censo_estado CHECK (estado IN ('HABILITADO', 'EXCLUIDO', 'EXENTO')),
	CONSTRAINT ck_registro_censo_causal_estado CHECK (
...
CREATE TABLE IF NOT EXISTS gestion_pre_electoral.candidaturas (
	...
	fecha_actualizacion TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	CONSTRAINT uk_candidatura_eleccion_documento UNIQUE (eleccion_id, documento),
	CONSTRAINT fk_candidatura_reemplazada FOREIGN KEY (candidatura_reemplazada_id)
		REFERENCES gestion_pre_electoral.candidaturas (id),
	CONSTRAINT ck_candidatura_estado CHECK (estado IN ('INSCRITA', 'EN_REVISION', 'ACEPTADA', 'RECHAZADA', 'REVOCADA', 'REEMPLAZADA')),
	CONSTRAINT ck_candidatura_motivo_reemplazo CHECK (
		motivo_reemplazo IS NULL OR motivo_reemplazo IN ('MUERTE', 'INCAPACIDAD')
	)
);
